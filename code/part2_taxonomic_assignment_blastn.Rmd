...
tax_df <- taxonomy %>%
  ...
  filter(perc_id > 96) %>% # blast output includes as low as 96 perc_id, but for now i only care about the top 2%
...
in_range <- in_range %>%
  ...
  filter(Species != "Micropterus floridanus") %>%
  filter(Species != "Rhinogobiops nicholsii") %>% 
  filter(Species != "Sardinops sagax") 
## likely more could be filtered out here- but lots of these don't make it into final taxonomic output so will take a closer look later on
```
...
## ASVs with >99% match 
```{r}
match_99_gadidae <- seq_id_diff_gadidae %>%
  filter(top_perc >= 99) %>%
  select(qseqid) %>%
  unique()

match_lessthan99_gadidae <- seq_id_diff_gadidae %>%
  filter(top_perc < 99) %>%
  select(qseqid) %>%
  unique()
```
...
```{r}
# filter out any taxa that are >1% less matching identity than the top taxonomic hit for a given sequence
match_99_to_remove_low_perc_hits_gadidae <- seq_id_diff_gadidae %>%
  ungroup() %>%
  filter(qseqid %in% match_99_gadidae$qseqid) %>%
  group_by(qseqid) %>%
  filter(diff > 0.5)

match_lessthan99_to_remove_low_perc_hits_gadidae <- seq_id_diff_gadidae %>%
  ungroup() %>%
  filter(qseqid %in% match_lessthan99_gadidae$qseqid) %>%
  group_by(qseqid) %>%
  filter(diff > 1)

to_remove_low_perc_hits_gadidae <- match_99_to_remove_low_perc_hits_gadidae %>%
  bind_rows(match_lessthan99_to_remove_low_perc_hits_gadidae)

keepers_gadidae <- seq_id_diff_gadidae %>%
  anti_join(to_remove_low_perc_hits_gadidae) %>%
  filter(top_perc > 98)
```
...
## ASVs with >99% match 
```{r}
match_99_salmon <- seq_id_diff_salmon %>%
  filter(top_perc >= 99) %>%
  select(qseqid) %>%
  unique()

match_lessthan99_salmon <- seq_id_diff_salmon %>%
  filter(top_perc < 99) %>%
  select(qseqid) %>%
  unique()
```
...
```{r}
# filter out any taxa that are >1% less matching identity than the top taxonomic hit for a given sequence
match_99_to_remove_low_perc_hits_salmon <- seq_id_diff_salmon %>%
  ungroup() %>%
  filter(qseqid %in% match_99_salmon$qseqid) %>%
  group_by(qseqid) %>%
  filter(diff > 0.5)

match_lessthan99_to_remove_low_perc_hits_salmon <- seq_id_diff_salmon %>%
  ungroup() %>%
  filter(qseqid %in% match_lessthan99_salmon$qseqid) %>%
  group_by(qseqid) %>%
  filter(diff > 1)

to_remove_low_perc_hits_salmon <- match_99_to_remove_low_perc_hits_salmon %>%
  bind_rows(match_lessthan99_to_remove_low_perc_hits_salmon)
