---
title: "decontamination of pcr replicates and ASVs for 2021 GOA mifish samples"
...
```{r}
metadata <- read.csv("/home/kimberly.ledger/GOA2021_mifish_mb/data/GOA2021_metadata_20241107.csv")
...
```{r}
asv_table <- readRDS("/home/kimberly.ledger/GOA2021_mifish_mb/data/dadasnake_20241106/filtered.seqTab.RDS") %>%
  select(!Row.names)
...
asv_table_with_sample_type %>%
  ...
  filter(sample_type %in% c("positive_control")) %>%
  filter(reads > 0) %>%
...
```{r}
taxonomy <- read.csv("/home/kimberly.ledger/GOA2021_mifish_mb/outputs/taxonomy_20241107_collapsed.csv") %>%
  select(!X) %>%
  rename(ASV = qseqid)
...
based on taxa accumulation curve and summary table, we will remove any pcr replicate with fewer than 500 reads from downstream analyses
...
normalized <- asv_table_filter4 %>%
  filter(!Sample_ID %in% c("e00512-C", "e00562-A")) %>% 
...
normalized_w_meta <- normalized %>%
  left_join(metadata) %>%
  filter(!is.na(location1)) %>%   ## remove the samples without location data and lat/lon (it is possible to add it a dummpy variable to group the biological replicates to that go together from these unknown locations, but i imagine we won't use these samples if we don't know where they came from)
...
```{r}
write.csv(asv_table_filter6, "/home/kimberly.ledger/GOA2021_mifish_mb/outputs/decontaminated_asv_table.csv")
```
